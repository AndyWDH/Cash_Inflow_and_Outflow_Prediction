...
def build_optimized_neural_network(input_dim):
    ...
    # 输出层
    outputs = tf.keras.layers.Dense(2)(x)  # 输出purchase和redeem两个值
...
def prepare_training_data(df):
    ...
    # 输出训练数据的基本统计信息
    print(f"训练数据统计 - Purchase: [{daily_summary['total_purchase_amt'].min():.2f}, {daily_summary['total_purchase_amt'].max():.2f}], Mean: {daily_summary['total_purchase_amt'].mean():.2f}")
    print(f"训练数据统计 - Redeem: [{daily_summary['total_redeem_amt'].min():.2f}, {daily_summary['total_redeem_amt'].max():.2f}], Mean: {daily_summary['total_redeem_amt'].mean():.2f}")
    ...
    # 输出数据形状和范围
    print(f"特征数据形状: X={X.shape}, 目标数据形状: y={y.shape}")
    print(f"特征范围 - X: [{X.min():.3f}, {X.max():.3f}]")
    ...
    print(f"标准化后特征范围 - X_scaled: [{X_scaled.min():.3f}, {X_scaled.max():.3f}]")
    print(f"标准化后目标范围 - y_scaled: [{y_scaled.min():.3f}, {y_scaled.max():.3f}]")
...
def main():
    ...
    output_file = os.path.join(script_dir, 'final_optimized_nn_result.csv')
