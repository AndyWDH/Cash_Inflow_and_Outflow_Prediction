...
for idx, row in result_df.iterrows():
    ...
    # 预测值 = 基础平均值 × weekday因子 × day因子（乘法关系）
...
save_file_path = os.path.join(script_dir, 'result2.csv')
# 保存结果到CSV文件，不包含表头
result_df.to_csv(save_file_path, header=False, index=False)

print("周期因子预测结果已保存到 result2.csv（不包含表头）")
